...
//
//    public void initialization() throws WrongSettingException, SingleMatchException, WrongSettingException, SingleMatchException {
//        List<Player> players = new ArrayList<>(List.of(new Player("player1"), new Player("player2"), new Player("player3")));
//        this.match = new MultiMatch(players);
...
//
//        @Test
//        public void testWareHouseInsertion() throws WrongSettingException, SingleMatchException, NegativeQuantityException, NotEnoughResourcesException, InvalidCardRequestException {
//            initialization();
//
...
//            WarehouseInsertionMessage message = new WarehouseInsertionMessage("player1",new ArrayList<>(Arrays.asList(resource,resource1,new PhysicalResource(ResType.COIN,2))));
//            WarehouseInsertionMessage message1 = new WarehouseInsertionMessage("player1",new ArrayList<>(Arrays.asList(resource)));
//
//            Player p = match.getCurrentPlayer();
//
//            p.addToWarehouse(resource);
//            p.addToWarehouse(resource1);
//
//            controller.setCurrentState(StateName.INTERMEDIATE);
//            controller.newOperation(message);
//
//            assertEquals(resource,p.getPersonalBoard().getWarehouse().getWarehouseDisposition().get(1));
//            assertEquals(resource.getQuantity(),p.getPersonalBoard().getWarehouse().getWarehouseDisposition().get(1).getQuantity());
//            assertEquals(resource1,p.getPersonalBoard().getWarehouse().getWarehouseDisposition().get(0));
//            assertEquals(StateName.END_TURN,controller.getCurrentState());
//
//            p.payFromWarehouse(resource,2);
//            p.payFromWarehouse(resource1,1);
//
//            p.addToWarehouse(resource);
//            p.addToWarehouse(resource1);
//
//            controller.setCurrentState(StateName.INTERMEDIATE);
//            controller.newOperation(message1);
//
//            assertEquals(StateName.NOT_DISCARDABLE_RESOURCES,controller.getCurrentState());
//
//        }
//
//
